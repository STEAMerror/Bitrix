								...
								<div class="bx_catalog_item_scu wrapper_sku" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PROP_DIV']; ?>">
									<?foreach ($arSkuTemplate as $code => $strTemplate){
                                        ...
                                        foreach($arResult["OFFERS"] as $itemOffer):
                                            ...
                                            $button_in_store = $button_in_store . '<a rel="nofollow" href="' . $itemOffer["PROPERTIES"]["URL2"]["VALUE"]. $referer .'"  class="btn btn-default basket read_more"><span>' . GetMessage("OFO") . '</span></a>';
                                        endforeach;

                                        if(isset($arResult['OFFERS'][0]['PROPERTIES']['URL']['VALUE'])):
                                            if(strlen($arResult['OFFERS'][0]['PROPERTIES']['URL']['VALUE'])>0):
                                                $strTemplate = str_replace('</li>','</li><div style="display: inline-block;margin-top: 20px;">' . $button_in_store . '</div>',$strTemplate);
                                            endif;
                                        endif;

                                        //if(strpos($APPLICATION->GetCurPage(), "finansy")!=false  || strpos($APPLICATION->GetCurPage(), "strakhovanie")!=false):
                                            $strTemplate = str_replace("Магазин","",$strTemplate);
                                        //endif;
            ...
            <div class="tabs">
                <ul class="nav nav-tabs">
					...
					<a href="#review" data-toggle="tab"><span><?//=($arParams["TAB_REVIEW_NAME"] ? $arParams["TAB_REVIEW_NAME"] : GetMessage("REVIEW_TAB"))?></span><span class="count empty"></span></a>
                        ...
                        <?if(!empty($arResult["OFFERS"][0]["PROPERTIES"]["PREIMUSHCHESTVA"]["VALUE"])):?>
                        ...
                        <?endif;?>
                        <?if(!empty($arResult["OFFERS"][0]["PROPERTIES"]["TREBOVANIYA"]["VALUE"])):?>
                            <li class="product_trebovaniya_tab<?=(!($iTab++) ? ' active' : '')?>">
                                <a href="#trebovaniya" data-toggle="tab"><span><?=GetMessage("TR_TAB")?></span><span class="count empty"></span></a>
                            </li>
                        <?endif;?>
                    <?endif;?>
                ...
                <div class="tab-content">
                    ...
                    <?if($arParams["USE_REVIEW"] == "Y"):?>
                        ...
                        <?if(!empty($arResult["OFFERS"][0]["PROPERTIES"]["PREIMUSHCHESTVA"]["VALUE"])):?>
                            <div class="tab-pane <?=(!($iTab++) ? 'active' : '')?>" id="preimushchestva">
                                ...
                                <div class="detail_text">
                                    <?
                                    $abz = explode ("\n",$arResult["OFFERS"][0]["PROPERTIES"]["PREIMUSHCHESTVA"]["VALUE"]);
                                    for($ii=0;$i<count($abz);$i++):?>
                        ...
                        <?endif;?>
                        <?if(!empty($arResult["OFFERS"][0]["PROPERTIES"]["TREBOVANIYA"]["VALUE"])):?>
                            <div class="tab-pane <?=(!($iTab++) ? 'active' : '')?>" id="trebovaniya">
                                <div class="title-tab-heading visible-xs">
                                    <?=GetMessage("TR_TAB")?>
                                </div>
                                <div class="detail_text">
                                    <?=$arResult["OFFERS"][0]["PROPERTIES"]["TREBOVANIYA"]["VALUE"];?>
                                </div>
                            </div>
                        <?endif;?>
                    <?endif;?>
